...
{
     ScopedLock rwl(&reply_window_m_);
     list<reply_t> ::iterator iter = reply_window_[clt_nonce].begin();
		...
		unsigned int xid_rep, char **b, int *sz)
{
    ...
    {
        if(iter->xid<xid_rep)
        {//iter automatically goes to the nex iterator of the list 
        ...
        else
        {
            ...
            //it ensures the xid is not forgotten.
            else if(iter->xid<xid)
            {
                beforexid_rep = false;
            }
            iter++;
